     ...
     if (mode == "make_base"sv) {
        {
            ...
            reader.ReadRoutingSettings(doc), cat);
            reader.Fill(cat, transport_router);
            graph::Router<BusRouteWeight> router(transport_router.GetRouteGraph<BusRouteWeight>());
            Serialize::Serializer serialize( cat,transport_router,reader.GetRenderSettings(),reader.ReadSerializeSettings(doc), router);
            serialize.Save();
        }
